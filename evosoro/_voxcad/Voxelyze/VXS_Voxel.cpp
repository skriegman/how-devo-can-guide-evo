...
//		if(pSim->IsMaxVelLimitEnabled()){ //check to make sure we're not going over the speed limit!
	...
	// SCALE
	vfloat maxScale = pSim->pEnv->getUsingKramerFabric() ? GetNominalSize() : (1+pSim->pEnv->getGrowthAmplitude())*GetNominalSize();  // max size is specified by growth amplitude if not Kramer Fabric
    ...
    if (pSim->pEnv->pObj->GetUsingInitialVoxelSize() || pSim->pEnv->pObj->GetUsingFinalVoxelSize())
    {
        ...
        currSize = (1+PreNatalTempFrac)*(1+DevTempFact)*GetNominalSize();
